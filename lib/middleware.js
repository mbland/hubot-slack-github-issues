...
Middleware.prototype.execute = function(message, postResponse) {
  var errorMessage;

  try {
    return doExecute(this, message, postResponse);
...
function doExecute(middleware, message, postResponse) {
  var rule = middleware.findMatchingRule(message),
      msgId,
      finish;
  ...
  finish = handleFinish(msgId, middleware, postResponse);
...
function handleFinish(messageId, middleware, postResponse) {
  return function(message) {
