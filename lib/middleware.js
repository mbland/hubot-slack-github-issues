...
function doExecute(message) {
  ...
  var msgId = message.item.channel + ':' + message.item.ts,
      channel = this.slackClient.getChannel(message.item.channel)
        .catch(errHandler.bind(this, msgId, 'failed to get channel info')),
      team = this.slackClient.getTeam()
  ...
  processMessage = function(msgId, message, values) {
    var channel = values.shift(),
        ...
        permalink = 'https://' + team.domain + '.slack.com/archives/' +
    ...
    this.logger.info(msgId, 'processing:', permalink);
  ...
  return Promise.all([channel, team, lock])
    .catch(releaseLockOnError.bind(this, msgId, lock))
...
function releaseLockOnError(msgId, lock, err) {
  ...
  return lock.then(releaseLockIfAcquired.bind(this)).then(function() {
    throw err;
  });
}
