...
Middleware.prototype.execute = function(message, done) {
  var errorMessage;

  try {
    return doExecute(this, message, done);
...
function doExecute(middleware, message, done) {
  ...
  finish = handleFinish(msgId, middleware, done);
...
function handleFinish(messageId, middleware, done) {
  return function(message) {
