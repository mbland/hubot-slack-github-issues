...
Middleware.prototype.execute = function(response) {
  var errorMessage;

  try {
    return doExecute(this, response);
    ...
    response.reply(errorMessage);
  }
};

function doExecute(middleware, response) {
  ...
  finish = handleFinish(msgId, middleware, response);
...
function handleFinish(messageId, middleware, response) {
  return function(message) {
