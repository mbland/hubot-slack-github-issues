...
function doExecute(middleware, message) {
  var channelName = middleware.slackClient.getChannelName(message.item.channel),
  ...
  finish = handleFinish(msgId, middleware);

  return getReactions(middleware, msgId, channelName, teamDomain, message)
...
function getReactions(middleware, msgId, channelName, teamDomain, message) {
...
function handleFinish(messageId, middleware) {
  return function(message) {
