...
function fileIssue(msgId, channel, rule, getReactionsResponse) {
  var metadata;

  if (alreadyProcessed(getReactionsResponse, this.successReaction)) {
    this.logger.info(msgId, 'already processed');
    return Promise.reject(null);
  }

  metadata = this.parseMetadata(getReactionsResponse, channel);
...
function alreadyProcessed(getReactionsResponse, successReaction) {
