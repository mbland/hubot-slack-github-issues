...
Impl.prototype.execute = function(resolve, reject) {
  var impl = this,
      channel = this.filer.slackClient.channelInfo(this.message.item.channel)
        .catch(this.abort('failed to get channel info')),
      team = this.filer.slackClient.teamInfo()
...
Impl.prototype.abort = function(label) {
  ...
  return function(err) {
    ...
    impl.filer.logger.error(impl.msgId, err.message);
    throw err;
  };
};
...
Impl.prototype.processMessage = function(channel, team) {
  ...
  this.team = team,
  this.rule = this.filer.findMatchingRule(this.message, this.channel.name);

  if (!this.rule) {
    return null;
  }
  return this.filer.messageLock.lock(this.msgId)
...
Impl.prototype.fileIssueIfNoSuccessReaction = function(lockAcquired) {
  ...
  if (!lockAcquired) {
    this.filer.logger.info(this.msgId, 'already in progress');
    return null;
  }
  this.filer.logger.info(this.msgId, 'processing:',
    this.filer.slackClient.permalink(this.team, this.channel, this.message));
  this.filer.logger.info(this.msgId, 'matches rule:',
    this.rule.toLogString());
  finish = function(result) {
...
Impl.prototype.releaseLock = function(result) {
  return this.filer.messageLock.unlock(this.msgId)
...
Impl.prototype.getReactions = function() {
  ...
  this.filer.logger.info(this.msgId, 'getting reactions');
  return this.filer.slackClient.getReactions(channelId, timestamp)
...
Impl.prototype.fileIssue = function(getReactionsResponse) {
  ...
  if (this.alreadyProcessed(getReactionsResponse)) {
    this.filer.logger.info(this.msgId, 'already processed');
    return Promise.reject(null);
  }

  metadata = this.filer.parseMetadata(getReactionsResponse, this.channel);
  this.filer.logger.info(this.msgId, 'filing GitHub issue in ' +
    this.filer.githubClient.user + '/' + githubRepository);

  return this.filer.githubClient.fileNewIssue(metadata, githubRepository)
...
Impl.prototype.addSuccessReaction = function(issueUrl) {
  ...
  this.filer.logger.info(this.msgId, 'adding', this.successReaction);

  return this.filer.slackClient.addSuccessReaction(item.channel, item.ts)
    .catch(this.abort('created ' + issueUrl + ' but failed to add ' +
      this.successReaction))
    .then(function() {
      impl.filer.logger.info(impl.msgId, 'created: ' + issueUrl);
      return issueUrl;
    });
};
