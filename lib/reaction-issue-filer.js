...
function doExecute(msgId, message) {
  ...
  return Promise.all([channel, team])
    .then(processIfMessageMatchesRule.bind(this, msgId, message));
}

function processIfMessageMatchesRule(msgId, message, values) {
  ...
  return Promise.all([team, lock])
    .then(processMessageIfLockAcquired.bind(this, msgId, message, channel))
    .then(finish, finish);
}

function processMessageIfLockAcquired(msgId, message, channel, values) {
